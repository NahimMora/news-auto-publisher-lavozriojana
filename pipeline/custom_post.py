...
def render_preview_image(item: dict) -> bytes:
    ...
    from layout.image_generator import generate_instagram

    img = generate_instagram(item)
    buf = io.BytesIO()
...
def publish_custom_post(item: dict, *, log=None) -> dict:
    ...
    result = {"web_ok": False, "ig_ok": False, "fb_ok": False, "public_url": "", "error": None}

    if _dry_run_enabled():
        log("DRY-RUN activo (CUSTOM_POST_DRY_RUN=true): no se publica de verdad.")
        result.update(web_ok=True, ig_ok=True, fb_ok=True, public_url="https://example.com/dry-run-preview")
        return result
    ...
    try:
        result["ig_ok"] = post_to_instagram(item)
    except Exception as exc:
    ...
    try:
        result["fb_ok"] = post_to_facebook(item)
    except Exception as exc:
